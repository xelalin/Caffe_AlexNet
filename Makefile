...
##############################
# Derive FPGA include and lib directories
##############################
FPGA_INCLUDE_DIR := $(FPGA_DIR)/include
FPGA_LIB_DIR :=
# add <fpga>/lib64 only if it exists
ifneq ("$(wildcard $(FPGA_DIR)/lib/x86_64)","")
	FPGA_LIB_DIR += $(FPGA_DIR)/lib/x86_64
endif
FPGA_LIB_DIR += $(FPGA_DIR)/lib

ifeq ($(USE_FPGA), 1)
	INCLUDE_DIRS += $(FPGA_INCLUDE_DIR)
	LIBRARY_DIRS += $(FPGA_LIB_DIR)
	LIBRARIES += lmx6.0 xblas zmq xilinxopencl pthread rt
endif
ifeq ($(USE_XDNN), 1)
	LIBRARIES += xdnn
endif
